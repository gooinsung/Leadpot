...
//
// ⚠️ 공개(비로그인) API 는 @leadpot/public-ui 에 있다(SSR renderer 와 공유, docs/SSR-LANDING-PLAN.md §5-2).
// 이 파일은 그 중 프론트 자체에서 쓰는 것들을 재-export 하고, 인증이 필요한 관리자용 API 만 새로 정의한다.
import {
  setApiBaseUrl,
  ApiError,
  type ApiErrorBody,
...
// 위 목록은 이 파일 안에서도(Lead·LandingInput 등 관리자 타입이 참조) 실제로 쓰는 것들만 로컬로 들여온다.
// BlockType·FormBlock·FormSource·ConsentItem·LeadSubmitInput·LandingBlockType·LandingLive·RecordEventInput 는
// 이 파일 안에서는 안 쓰지만, 기존 프론트 코드의 `from "../api/client"` 임포트를 유지하려고 아래에서 재-export 만 한다.
export {
  ApiError,
  getPublicForm,
  submitLead,
...
setApiBaseUrl(BASE_URL); // @leadpot/public-ui 의 공개 API 도 같은 서버 주소를 쓰게 맞춘다
...
// ---------- 공통 에러 ----------
// ApiError·ApiErrorBody 는 @leadpot/public-ui 것을 그대로 쓴다(위에서 import+재-export) —
// 공개 API 가 던지는 에러와 관리자 API 가 던지는 에러가 같은 클래스여야 `instanceof ApiError` 가 일관되게 동작한다.
...
// ---------- 리드폼(Form) ----------
// FormType·BlockType·FormBlock·FormSource·FormInput·FormDetail 는 @leadpot/public-ui 것을 재-export(위)한다.
...
// ConsentItem·ConsentDocument 는 @leadpot/public-ui 것을 재-export(위)한다.
...
// getPublicConsentDoc 은 @leadpot/public-ui 것을 재-export(위)한다.
...
// ---------- 리드(수집 데이터) ----------
// LeadAnswer·LeadConsent 는 @leadpot/public-ui 것을 재-export(위)한다.
...
// LeadSubmitInput·getPublicForm·submitLead 는 @leadpot/public-ui 것을 재-export(위)한다.
...
// ---------- 랜딩페이지 ----------
// LandingBlockType·LandingBlock·PublicLanding 은 @leadpot/public-ui 것을 재-export(위)한다.
...
// LandingLive·getLandingLive 는 @leadpot/public-ui 것을 재-export(위)한다.
...
// recordVisit·RecordEventInput·recordEvent·recordEventBeacon 은 @leadpot/public-ui 것을 재-export(위)한다.
