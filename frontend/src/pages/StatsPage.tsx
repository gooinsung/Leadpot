...
export function StatsPage() {
  ...
  const empty = stats && stats.summary.visits === 0 && stats.summary.leads === 0;
      ...
      <main className="wrap dashboard">
        ...
        {loading && !stats ? (
          ...
          <div style={{ opacity: loading ? 0.55 : 1, transition: "opacity .15s" }}>
            {/* 요약 */}
            <div className="kpis" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", marginTop: 20 }}>
              <div className="kpi card"><div className="k-label">유입(방문)</div><div className="k-val">{stats.summary.visits.toLocaleString("ko-KR")}</div></div>
              ...
              <div className="kpi card"><div className="k-label">전환율</div><div className="k-val">{stats.summary.conversionRate}<span style={{ fontSize: 16 }}>%</span></div></div>
              <div className="kpi card"><div className="k-label">기간</div><div className="k-val" style={{ fontSize: 15 }}>{stats.from} ~ {stats.to}</div></div>
            </div>
            ...
            <section className="card card-pad">
              <div className="card-h" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span>일별 추이</span>
                <span className="chart-legend"><i className="lg lg-v" />방문 <i className="lg lg-l" />리드</span>
              </div>
...
function DayChart({ data }: { data: StatDayPoint[] }) {
  const max = Math.max(1, ...data.map((d) => Math.max(d.visits, d.leads)));
  return (
    <div className="day-chart">
      {data.map((d) => (
        <div className="day-bar-wrap" key={d.date} title={`${d.date}\n방문 ${d.visits} · 리드 ${d.leads}`}>
          <div className="day-bar day-bar-v" style={{ height: `${(d.visits / max) * 100}%` }} />
          <div className="day-bar day-bar-l" style={{ height: `${(d.leads / max) * 100}%` }} />
        </div>
      ))}
    </div>
  );
      ...
      {rows.length === 0 ? (
        ...
        <div className="stats-table-scroll">
          <table className="stats-table">
            <thead>
              <tr><th>이름</th><th className="num">방문</th><th className="num">리드</th><th className="num">전환율</th></tr>
            </thead>
            <tbody>
              {rows.map((r, i) => (
