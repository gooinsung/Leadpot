      ...
      <main className="wrap dashboard client-wrap">
        ...
        <div className="card card-pad" style={{ marginBottom: 16 }}>
          ...
          <ol className="dash-sub" style={{ margin: 0, paddingLeft: 20, lineHeight: 1.9 }}>
            ...
            <li>리드 카드를 아무 곳이나 누르면(전화·상태 부분만 빼고) 상세 화면이 열립니다. 전체 답변·메모·이력을 여기서 확인합니다.</li>
            <li>
              카드의 <b>📞 전화</b> 버튼을 누르면 <b>휴대폰</b>에서는 곧바로 전화 앱이 열려 통화로
              연결됩니다. <b>PC(컴퓨터)에서 보고 있다면 눌러도 전화가 걸리지 않습니다</b> — 컴퓨터와
              휴대폰이 서로 연결된 게 아니라, 휴대폰 자체의 전화 기능을 부르는 방식이라서입니다.
              통화하시려면 휴대폰으로 접속해 주세요.
            </li>
        ...
        <div className="card card-pad" style={{ marginBottom: 16 }}>
          ...
          <p className="dash-sub" style={{ marginTop: 0 }}>
            상태는 담당 마케터와 <b>실시간으로 공유</b>됩니다. 누가 언제 상태를 바꿨는지는 리드
            상세의 <b>메모 · 이력</b>에 전부 날짜와 함께 자동으로 남으니, 나중에 "내가 언제 이걸
            바꿨더라"를 언제든 다시 확인할 수 있습니다. 기본 상태는 4가지입니다.
          </p>
          <ul className="dash-sub" style={{ margin: 0, paddingLeft: 20, lineHeight: 1.9 }}>
            <li><b>신규</b> — 방금 들어온 리드. 아직 아무 처리도 안 한 상태.</li>
            <li><b>유효</b> — 정상적인 리드로 확인됨. 이 상태가 되면 <b>확정</b>됩니다.</li>
            <li><b>AS요청</b> — 내가 이의를 제기해 담당자가 확인 중인 상태(아래 3번).</li>
            <li><b>무효</b> — 잘못된 리드로 확정. <b>담당 마케터만</b> 지정할 수 있습니다.</li>
          </ul>
          <p className="dash-sub">
            여기에 더해 <b>나만의 상태</b>(상담중, 부재 3일차, 상담거부 등)를{" "}
            <Link to="/client/integrations">설정 → 진행상태 관리</Link>에서 직접 만들어 쓸 수 있습니다.
            예를 들어 신규 → <i>1차콜</i> → <i>문자 발송</i> 처럼 나만의 순서로 정리할 수 있고, 이렇게
            바뀐 기록도 전부 위 메모 · 이력에 남습니다.
          </p>
          <p className="dash-sub" style={{ marginTop: 0 }}>
            <b>⏰ 자동 승인 안내</b> — 담당 마케터가 리드폼에 자동 승인 기간(예: 7일)을 설정해두면,
            접수 후 그 기간이 지나도록 아무 처리도 하지 않은 리드는 자동으로 <b>유효</b>로 확정됩니다.
            (예: 자동 승인이 7일이면 접수 8일째 되는 날 자동으로 유효 처리) 이의가 있는 리드는
            <b> 기간이 지나기 전에 미리</b> AS 요청을 넣어주세요 — 자동 승인된 뒤에는 아래 방법으로만
            바로잡을 수 있습니다.
          </p>
          <p className="dash-sub" style={{ marginBottom: 0 }}>
            <b>⚠️ 유효가 된 리드는 상태를 직접 바꿀 수 없습니다.</b> 확정된 상태를 실수로 되돌리는 걸
            막기 위함입니다. 유효로 확정된 리드의 상태를 바꾸고 싶다면 반드시 아래 <b>3번 AS 요청</b>으로
            담당 마케터에게 요청해 주세요 — 마케터가 인정하면 무효로, 거부하면 유효로 그대로 확정됩니다.
          </p>
        </div>
        ...
        <div className="card card-pad" style={{ marginBottom: 16 }}>
          ...
          <ol className="dash-sub" style={{ margin: 0, paddingLeft: 20, lineHeight: 1.9 }}>
            ...
            <li>담당자가 <b>인정</b>하면 무효, <b>거부</b>하면 유효로 확정됩니다. 결과는 리드 상세의 AS 이력에서 확인합니다.</li>
          </ol>
        </div>
