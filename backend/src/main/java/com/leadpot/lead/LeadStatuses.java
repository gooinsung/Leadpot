 ...
 * <li>{@link #VALID} 유효 — <b>확정 판정</b>. 자동 승인의 목표 상태이기도 하다. 확정된 뒤에는
 *     광고주가 직접 되돌릴 수 없고 AS 요청으로만 바꿀 수 있다(사용자 확정).</li>
 * <li>{@link #AS_REQUESTED} AS요청 — 광고주가 사유·증빙과 함께 이의를 제기한 상태.
 *     <b>일반 상태 변경으로는 진입할 수 없고</b> AS 요청 플로우로만 만들어진다.
 *     마케터가 인정(→무효)/거부(→유효)로 해소한다.</li>
 * <li>{@link #INVALID} 무효 — <b>넣는 것도 빼는 것도 마케터만</b> 할 수 있다(사용자 확정).</li>
