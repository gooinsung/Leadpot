...
public class SiteIpBlockService {
    ...
    /** 로그 조회 최대 건수(과도한 조회 방지) — 리드폼별 차단 로그와 같은 기준. */
    private static final int MAX_HITS = 500;

    private final SiteIpBlockRepository repository;
    private final SiteIpBlockHitRepository hitRepository;

    public SiteIpBlockService(SiteIpBlockRepository repository, SiteIpBlockHitRepository hitRepository) {
    ...
    public boolean isBlocked(Long userId, String ip) {
        return blockedPattern(userId, ip) != null;
    }

    /** 걸린 규칙(pattern)을 돌려준다. 안 걸리면 null. 로그에 어떤 규칙이 걸렸는지 남기려고 분리. */
    @Transactional(readOnly = true)
    public String blockedPattern(Long userId, String ip) {
        if (userId == null || ip == null || ip.isBlank()) {
            return null;
        }
        for (SiteIpBlock r : repository.findByUserIdOrderByCreatedAtDesc(userId)) {
     ...
     * 차단된 접속 시도를 남긴다.
     * <p>
     * {@code REQUIRES_NEW} — 리드 제출처럼 바깥 트랜잭션이 롤백되는 흐름에서도 로그는 남아야 한다.
     * 로그 실패가 본 동작을 막아서는 안 되므로 예외는 삼킨다.
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void recordHit(Long userId, String ip, String matchedPattern, SiteIpBlockHit.Source source,
            String userAgent) {
        try {
            hitRepository.save(new SiteIpBlockHit(userId, cut(ip, 64), cut(matchedPattern, 64), source,
                    cut(userAgent, 1024)));
        } catch (RuntimeException e) {
            // 확인용 로그일 뿐이므로 실패해도 차단 자체는 그대로 동작한다.
        }
    }

    @Transactional(readOnly = true)
    public List<SiteIpBlockHitResponse> hits(Long userId) {
        return hitRepository.findByUserIdOrderByCreatedAtDesc(userId, PageRequest.of(0, MAX_HITS))
                .stream().map(SiteIpBlockHitResponse::from).toList();
    }

    @Transactional
    public void clearHits(Long userId) {
        hitRepository.deleteByUserId(userId);
    }
