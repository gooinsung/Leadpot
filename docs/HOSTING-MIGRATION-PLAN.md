...
### P3. **SSH 가 없어서 시크릿을 아예 못 고친다** — 지금 보안 이슈가 여기 막혀 있다

VM `~/Leadpot/.env` 는 자동 배포 대상이 아니라 SSH 로만 고칠 수 있다. 그런데 **어느 PC 에도 개인키가 없다**
(gooin PC·wincube PC 모두 `~/.ssh` 에 `known_hosts` 만 있음 — 2026-08-07 확인).

그 결과 PROGRESS.md 에 이렇게 걸려 있다:

> **"Neon 비밀번호가 대화에 평문 노출됐다. 재발급 필요.
> 재발급하면 VM `.env` 도 같이 바꿔야 운영이 안 죽는다 → SSH 확보와 묶어서 처리. (아직 안 함)"**

**즉 노출된 비밀번호를 못 바꾸고 있다.** Railway 로 옮기면 시크릿이 웹 UI 로 오므로 SSH 자체가 필요 없어지고,
**막혀 있던 Neon 비번 교체가 이전과 동시에 끝난다.** → §4-3
...
### 4-1. 🔴 R2 가 실제로 켜져 있는지 확인 — **SSH 없이 하는 법**
...
**SSH 없이 확인하는 법** — 이미지가 들어간 **공개 랜딩페이지를 열고 이미지 URL 을 본다**(브라우저 개발자도구 Network, 또는 이미지 우클릭 → 주소 복사):

| 보이는 URL | 판정 | 해야 할 일 |
|---|---|---|
| R2 공개 도메인 (`...r2.dev/...` 또는 전용 도메인) | ✅ **r2** | 없음. R2 값 5개만 Railway 로 |
| `https://api.lead-pot.com/uploads/...` | ❌ **local** | **이전 전에 R2 전환 선행** |

`local` 이면: R2 버킷 생성 → 환경변수 주입 → 재배포 → **기존 업로드 파일을 도커 볼륨에서 R2 로 복사**
(파일명이 UUID 라 덮어쓰기 충돌 없음). ⚠️ **이 복사에는 SSH 가 필요하다.**

### 4-2. 🔴 시크릿 조달 — VM `.env` 를 못 읽는다는 전제로 짠다

**SSH 개인키가 없다**(§P3). 다행히 **대부분의 값은 각 서비스 콘솔에서 새로 발급하면 된다:**

| 값 | 조달 방법 |
|---|---|
| Neon 접속정보 | **어차피 재발급해야 한다**(§4-3) → Neon 콘솔에서 새 비번 |
| R2 키 5개 | Cloudflare 콘솔에서 새 API 토큰 발급 + 버킷명·endpoint·공개 URL 확인 |
| Solapi 키 | 솔라피 콘솔에서 재발급 |
| CORS·공개 URL 등 | 이 문서 §6 에 값이 적혀 있음 |
| **`APP_JWT_SECRET`** | ⚠️ **콘솔에서 얻을 수 없다. 유일하게 VM 에만 있는 값** |

> #### ⚠️ `APP_JWT_SECRET` — 두 가지 길
> - **(권장) SSH 키를 확보해 기존 값을 가져온다** → 로그인 세션이 그대로 유지된다.
> - **못 하면 새로 만든다**(32바이트 이상 랜덤) → **전원 로그아웃.** 마케터·**광고주 전원이 다시 로그인**해야 한다.
>   공개 회원가입은 닫혀 있어(`APP_AUTH_SIGNUP_ENABLED=false`) 계정 수가 적으므로 감당은 되지만,
>   **광고주에게 미리 공지해야 한다.** 비밀번호는 안 바뀌고 재로그인만 하면 된다.
>
> ❗ **SSH 확보는 어차피 PROGRESS.md 의 미해결 과제 3번이다.** 이전과 묶어서 처리하는 게 가장 효율적이다.

### 4-3. Neon 비밀번호 재발급을 **이 이전과 함께** 한다 ⭐

노출된 비번을 못 바꾸고 있던 이유가 "VM `.env` 를 못 고쳐서"였다. **이전하면 그 제약이 사라진다:**

1. Neon 콘솔에서 `neondb_owner` 비번 **재발급**
2. **새 비번을 Railway 환경변수에만** 넣는다 (VM 은 옛 비번으로 계속 돎 — 롤백 경로 유지)
3. 컷오버 완료 + 관찰 끝 → VM 종료 → **옛 비번은 자동으로 무효**

> ⚠️ 2번 시점에 **VM 백엔드가 옛 비번으로 죽는다.** Neon 은 롤 단위로 비번을 바꾸므로 **옛 비번은 즉시 무효**다.
> → **비번 재발급은 Step A4(DNS 전환) 직전에** 하고, 롤백이 필요하면 VM `.env` 도 고쳐야 함을 각오할 것.
> **롤백 여지를 최대로 두려면**: 재발급을 **Step A5 관찰이 끝난 뒤**로 미루고, 이전 자체는 옛 비번으로 진행한다. **← 이쪽 권장**
...
- [ ] (선택) **Neon 비번 재발급**(§4-3) → Railway 환경변수 갱신
...
| `APP_JWT_SECRET` | ⚠️ **32바이트 이상.** 새로 만들면 **전원 로그아웃** — §4-2 |
...
| **전원 로그아웃** (JWT 시크릿 못 가져옴) | §4-2. SSH 확보를 먼저 시도하고, 안 되면 **광고주에게 사전 공지** |
...
2. **🔴 SSH 키 확보 여부** — 확보하면 JWT 시크릿을 그대로 옮겨 **로그인 세션 유지**. 못 하면 **전원 재로그인**(§4-2)
3. **§4-1 R2 판정** — 공개 랜딩 이미지 URL 이 R2 도메인인가, `api.lead-pot.com/uploads/` 인가?
4. **Phase B 를 같이 갈지, Phase A 만 먼저 할지** — **A 만 해도 P1·P2·P3 는 전부 해결된다.** B 는 VM 을 끄기 위한 마무리
...
```
[ ] §4-1 R2 판정 (공개 랜딩 이미지 URL 확인 — SSH 불필요)
[ ] §4-2 시크릿 조달 / SSH 확보 시도
