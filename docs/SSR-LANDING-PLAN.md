...
- [x] `/f/{id}` 단독 공개 폼도 SSR 로 통일(2026-09-08, §11 #7) — 로컬 검증 완료(SSR 렌더·404·
      `app` 다른 경로는 관리 앱으로 정상 프록시 유지), **실제 도메인 확인은 아직**(§9-C 로 이월)
- [ ] §9-C 남은 것: `/f/{id}` 실제 도메인에서 최종 확인, 외부 임베드 `embed.js` 정상
...
**C. 그 밖에**
- [ ] `app.lead-pot.com` 관리 화면 정상(§6-5 리버스 프록시 경유로 바뀜 — 로그인·대시보드·빌더 확인)
- [ ] `/f/{id}` 단독 공개 폼 — 이제 SSR(§11 #7). 실제 도메인에서 제출까지 정상 동작하는지 확인
- [ ] 외부 사이트 임베드(`embed.js`) 정상(안 건드린 영역, CSR 그대로)
...
| 4 | `/f/{id}` 단독 공개 폼 | ~~범위에서 제외~~ → **2026-09-08 SSR 범위에 포함**(아래 #7) | 처음엔 구글 광고 최종 URL 로 안 쓴다고 확인돼 범위에서 뺐으나, Phase 5 실제 배포까지 끝난 뒤 사용자가 "안 쓰는 페이지도 굳이 CSR로 남겨둘 이유가 없다"고 판단해 마저 SSR로 통일 |
...
| 7 | `/f/{id}` SSR 구현 방식(2026-09-08) | `renderer/src/app/f/[identifier]/page.tsx` 신설(서브도메인 랜딩과 같은 패턴 — `notFound()`·`error.tsx` SPA 폴백·`generateMetadata`). `proxy.ts` 가 `app.lead-pot.com/f/*` 만 예외적으로 이 렌더러 자체 라우팅으로 두고, 그 외 `app` 경로는 그대로 관리 앱(Pages)으로 리버스 프록시 | `/f/{id}`는 서브도메인이 없어 계정과 무관하게 폼 번호만으로 전역 조회되므로 로직이 단순함. 방문기록·픽셀초기화·렌더는 새 공유 컴포넌트 `PublicFormPageView`(`packages/public-ui`)로 뽑아 frontend(`/f/:id`, 로컬 개발·이중 방어용으로 유지)와 renderer 가 함께 씀 |

> 처음엔(#4) 이 계획의 범위가 서브도메인 랜딩만이었지만, Phase 5 실배포 검증까지 끝낸 뒤(#7) 나머지
> 공개 렌더링 경로(`/f/{id}`)도 마저 SSR로 통일해 "공개 화면은 전부 SSR"이 됐다. `getPublicForm()`
> 도 `resolveSite`/`getLandingLive` 와 같은 `ForwardedRequestContext`(§6-1)를 받도록 확장해 폼 단위
> IP 차단이 SSR 에서도 올바르게 동작한다.
