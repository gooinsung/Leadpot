...
>
> ⚠️ **2026-09-07 갱신 — 이 부록도 이제 옛날 얘기다.** 백엔드는 Railway 로 컷오버됐고(2026-08-09),
> **DB 는 Neon 을 삭제하고 Railway Postgres 로 옮겼다**(비용 문제). VM 은 이제 프론트 정적 파일만
> 서빙하고, 아래 C-2 백엔드 배포(VM 컨테이너)는 **더 이상 의미가 없다** — Neon 접속정보가 죽어서
> VM 백엔드 컨테이너는 뜨자마자 DB 연결에 실패한다(서비스에는 영향 없음, 실제 트래픽은 Railway).
> 현재 실제 구성은 [CLAUDE.md](../CLAUDE.md) §2·§3·§6 을 볼 것 — 이 부록은 "예전엔 이렇게
> 했다"는 기록으로만 남겨둔다.
