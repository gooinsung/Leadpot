...
- **(2026-09-01 추가) 랜딩 트리거에 `fullscreen` 옵션**: CTA 클릭 시 오버레이(모달) 대신 **화면 전체를 채우는 풀스크린**으로 스텝이 진행된다(카드 경계 없이 진행바·질문·"다음" 버튼이 화면 상/하단에 고정). 랜딩의 커버 블록(이미지·텍스트) → CTA → 연결된 STEP 폼의 질문 순서대로 노출 → 마지막 연락처 입력·제출. 구현은 프론트 전용(`landing_pages.content_json`의 FORM 블록 `trigger` 값이 `inline\|overlay\|fullscreen` 중 하나, 백엔드 스키마 변경 없음).
...
interaction_events             -- 요소 상호작용 이벤트 (I4/I5/I6, Flyway V15 + V37)
  id
  owner_id → users
  landing_page_id, form_id     -- 어디서 발생했는지
  event_type                   -- form_open(오버레이 CTA 클릭) / scroll / page_exit 등
  target                       -- 클릭 대상 라벨(버튼 텍스트)
  scroll_depth                 -- 스크롤 도달 %(0~100, V37) — eventType="scroll" 에서 임계값(25/50/75/100) 기록
  duration_sec                 -- 체류 시간(초, V37) — eventType="page_exit" 에서 기록
  ip_hash                      -- 고유 방문자 추정용 SHA-256
  created_at
  -- 활용: 전환 퍼널(방문→폼 열기→접수) + 요소 클릭 집계 + 고객 여정(스크롤 깊이·체류시간·즉시이탈률, I6).
  -- 공개 POST /api/public/events(best-effort). 이탈(page_exit)은 navigator.sendBeacon 으로 전송(언로드 중 fetch 취소 방지).
...
- 로그인 / 회원가입 / 비밀번호 재설정
- 대시보드(요약: 유입/접수/전환)
- **폼 관리**: 폼 목록 / 폼 생성·편집(유형 선택 → BASIC 항목편집 or STEP 단계편집) / 미리보기 / **복사**(`POST /api/forms/{id}/duplicate` — 항목·설정 복제, 리드·웹훅·광고주 연결·폼별 IP차단 제외)
- **랜딩 관리**: 랜딩 목록·그룹 / 랜딩 편집(상단·하단 구성 + **폼 연결**) / **복사**(`POST /api/landings/{id}/duplicate` — 새 slug·비공개(draft)로 시작, 연결 리드폼은 원본과 같은 것 유지) / 설정(도메인·SEO·중복·차단·완료페이지)
- **리드(디비내역)**: 목록·검색·필터·상태변경·엑셀 내보내기 / **IP 차단 관리**(리드폼별 IP·CIDR 차단 + 차단 접속 로그, `/forms/{id}/ip-blocks`) / **외부 임베드 코드 복사**(M6, script+Shadow DOM 스니펫)
- **자동 승인 기간**(리드폼 옵션): 켜 두면 접수 후 지정 일수가 지나도 **신규·상담중**인 리드를 서버가 **완료**로 넘기고 리드 이력에 자동 메모를 남긴다. 불량·이미 완료·휴지통은 제외, 광고주 상태는 유지. **⚠️ 소급 적용하지 않는다** — 설정을 켠 시각 이후 접수분만 대상(상세: `AutoApproveSettings` 주석)
- **HTML 요소 라이브러리(M8)**: 재사용 HTML 조각 관리(`/html-components`) → 랜딩·리드폼 HTML 블록에 복사 삽입. **동적 요소**(실시간 신청수·남은자리·최근신청자 토스트·플로팅 헤더/푸터)는 랜딩 HTML 블록의 '동적 요소 삽입'에서 넣고, 공개 랜딩이 `data-lp-live` 마커를 `GET /api/public/landings/{id}/live`(연결폼 리드 수+최근 마스킹)로 채움
- 통계: 일별·전환·UTM
- 계정/요금(후기)
