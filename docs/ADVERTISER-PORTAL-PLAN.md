...
- [ ] 🚧 **막힘: 개발용 DB 필요** — 아래 §9-C 참고. V18 적용·회귀 스모크가 여기에 걸려 있다.
- [ ] **V18 적용** (개발 DB → 검증 → 배포 시 운영 DB)
- [ ] **회귀 스모크(필수)**: 마케터 로그인·폼CRUD·랜딩·공개폼 제출·리드목록/상태/엑셀·통계·연동 전부 정상
- [ ] `main` 병합·푸시 + `PROGRESS.md`·이 체크리스트 갱신

---

## 9-C. 🚧 블로커: 개발용 DB 분리 필요 (2026-07-30 발견)

현재 **로컬과 운영이 Neon DB 한 대를 공유**한다. 이 상태로 광고주 포털을 개발하면 두 가지 문제가 있다.

**① 운영 데이터 오염** — A2~A7 은 계정·초대·권한을 만드는 기능이다. 로컬에서 스모크 테스트할 때마다
가짜 광고주 계정·초대장·권한이 **운영 DB에 실제로 생긴다.** (PROGRESS 기록상 과거 테스트 데이터도 이미 남아 있음)

**② ⚠️ 운영 백엔드 기동 실패 위험** — 로컬에서 V18 을 Neon 에 적용하면,
VM 에서 돌고 있는 **구버전 백엔드의 jar 에는 V18 파일이 없는데 DB 이력에는 V18 이 적용된** 상태가 된다.
Flyway 는 이걸 "future migration" 으로 보고 기본값에선 경고만 내지만(`ignoreMigrationPatterns=*:future`),
버전에 따라 `validate` 실패로 **백엔드가 아예 기동하지 못할 수 있다.**
→ 즉 마이그레이션은 **코드 배포와 같은 시점에** 운영 DB에 적용되어야 한다(배포 시 새 jar 의 Flyway 가 실행).

### 해결책 (권장) = **Neon 개발 브랜치**
Neon 은 DB 브랜치 기능을 제공한다(무료 플랜 포함). 운영 DB의 복사본을 브랜치로 만들어
`backend/src/main/resources/application-local.properties` 의 접속정보를 그 브랜치로 바꾼다.

- 로컬 개발·마이그레이션 테스트가 **운영 데이터에 전혀 영향 없음**
- 운영 DB에는 **배포 시점에만** 마이그레이션이 적용됨 (정상적인 순서)
- A2~A7 전체 기간에 계속 필요하다(1회 세팅으로 끝)

> 준비물(사용자 작업): Neon 콘솔에서 브랜치 생성 → 접속 문자열 전달.
> 대안: 로컬 Postgres(Docker) — 이 PC는 Docker Desktop 이 응답하지 않아 실패(2026-07-30, 12분 대기 후 포기).
