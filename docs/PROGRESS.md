...
>
> **👉 다음 작업 (우선순위 순)**
> 1. **U4 마무리** — `/advertisers` 행 액션 8개 → 주요 2개 + `⋯` 더보기. 같은 패턴을 다른 목록에도.
> 2. **U5 공통 상태 UX** — 빈 화면·로딩·에러·토스트 일관화.
> 3. **U7 비주얼 컨셉 확정** — 컨셉 4선 중 선택 후 토큰 반영. (U6 광고주 포털은 실제 광고주가 붙을 때 해도 늦지 않다 — 사용자와 상의)
>
