...
- **✅ 기본형 리드폼 입력 항목에 라디오버튼·체크박스 유형 추가 (2026-09-23, 원격 세션, 사용자 지시)**:
  입력 항목 '유형'에 **라디오버튼(단일 선택)** · **체크박스(중복 선택)** 가 생겼다. 선택지 편집·'기본 선택'은
  기존 선택박스(`select`)와 같은 `options.choices`/`options.defaultIndex` 를 그대로 쓴다.
  - **저장 형식**: 리드 answers 의 value 는 고른 라벨을 선택지 순서대로 `", "` 로 이은 문자열(스텝형 다중 선택과 같은 형식),
    fieldType 은 `radio`/`checkbox`. **DB·백엔드 변경 없음**(서버 형식 검증은 이 유형에 제약이 없다).
  - **공개 폼**: 선택 상태는 `values` 에 라벨이 아니라 **인덱스("0,2")** 로 두고 접수할 때 라벨로 바꾼다 —
    라벨에 쉼표가 있어도 선택을 되짚을 수 있게. 필수면 "하나 이상 골랐는가"만 본다. 스타일은 스텝형 목록 선택(`.sfr-list`) 재사용.
  - 같은 편집기를 쓰는 스텝형 '연락처 단계' 입력 항목에서도 동작한다. 수기 등록 창은 라디오=드롭다운, 체크박스=체크 목록.
  - **⬜ 남은 것**: 브라우저 육안 확인(이 세션엔 백엔드 DB 없음). 스텝형 편집기 미리보기의 연락처 단계는
    선택박스와 마찬가지로 입력칸 모양으로만 보인다(기존 한계 그대로).
