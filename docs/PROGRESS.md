...
> ## 🔴 최우선 — 구글시트 서비스 계정 키를 서버에 넣어야 한다 (2026-08-11, 미완)
>
> 브랜치 `feature/sheets-service-account` (커밋 `0bdd16c`). **코드는 끝났고 키가 없어 아직 못 쓴다.**
...
> ### 남은 일 (순서대로)
>
> 1. **사용자가 GCP 콘솔에서 서비스 계정 생성** — 프로젝트 → Sheets API 사용 설정 →
>    서비스 계정(역할 없음) → **JSON 키** 발급. ⚠️ 키는 시크릿이라 git·코드에 두지 않는다.
> 2. **키를 서버 환경변수에 등록** — `GOOGLE_SHEETS_CREDENTIALS`.
>    JSON 원본도 받지만 줄바꿈 때문에 **base64 권장**(`base64 -w0 key.json`).
>    현재 배포(오라클 VM)는 `~/Leadpot/.env` 를 SSH 로 직접 고치고 재기동해야 한다(자동 배포 대상 아님).
>    → **호스팅 이전(Railway) 후에는 웹 UI 로 넣을 수 있다.** 이전이 임박했다면 그 뒤에 넣는 편이 낫다.
> 3. **실제 시트로 E2E 검증** — 시트 공유에 서비스 계정 이메일을 **편집자**로 추가 →
>    리드폼 편집 > 옵션 > 구글시트에 시트 주소 붙여넣기 → 저장 → **'시트 테스트 발송'**.
>    (`/integrations` 화면이 서비스 계정 이메일을 보여주고 복사 버튼도 있다. 키가 없으면 빨간 경고가 뜬다.)
> 4. **기존 연동 사용자에게 재설정 안내** — 웹앱 URL 방식은 더 이상 동작하지 않는다.
...
> - ❌ **실제 구글 시트에 쓰는 건 검증 못 했다** — 키가 없다. 위 3번이 첫 실검증이다.
>
> ### 이어받는 사람이 알아야 할 것
>
...
> ### 🔜 다음에 할 일 (우선순위 순)
>
> 1. **광고주 구글시트 연동 플로우 결정** — 사용자와 논의만 하고 미구현. 두 갈래:
>    ⓐ 광고주 포털에 시트 설정 UI 추가(반나절) ⓑ Apps Script → **구글 서비스계정 + Sheets API** 로 교체
>    (그러면 광고주는 "이 이메일을 편집자로 추가"만 하면 된다 — 큰 작업).
>    지금 구조는 **Apps Script 웹앱 URL 방식이라 '시트 ID만 받아서' 대신 연동해줄 수 없다.**
>    구조 확인 결과: **리드폼 1개 = 광고주 1명**(DB UNIQUE), **광고주 1명 = 리드폼 N개** 가능,
