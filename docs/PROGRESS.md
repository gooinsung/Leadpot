...
> ## ✅ 2026-08-30 — **광고주 리드 목록 기본값 '미확인' — 콜 우선순위**
>
> 클라우드 세션(브랜치 `feature/advertiser-unseen-first-leads`, `main`에서 분기 — 병합 대기 중).
...
> - **검증**: 백엔드 전체 테스트(신규 `unseenOnlyExcludesViewedLeads` 포함) + 프론트 tsc·vitest 81개·
>   build 전부 통과. **브라우저 실검증은 안 함**(클라우드 세션, Neon 접속정보 없음).
>
> **다음 후보**:
> - ⬜ 사용자 확인 후 `main` 병합 → 배포(스키마 변경 없어 무해, 배포 리스크 낮음).
> - ⬜ 브라우저 실검증(다음 세션): 미확인 카드 숫자·전체 체크박스 토글·리드 상세 열람 후 배지가
>   사라지는지(같은 페이지 새로고침 전까지는 목록에 남아있는 게 의도된 동작 — PROGRESS 참고).
>
> ---
>
