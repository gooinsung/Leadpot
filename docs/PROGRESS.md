...
> ### 👉 남은 일
>
> - ⛔ **브라우저 실확인을 못 했다** — 로컬 DB에 스텝형 폼이 없고 빌더는 로그인이 필요하다.
>   빌드·타입·테스트(49건)·백엔드 test 는 전부 통과. **다음 세션에서 로그인해 실제로 만들어 볼 것**:
>   리드폼 만들기 → 🧮 계산기 선택 → 저장 → 공개 URL 을 **모바일 375px** 로 확인(질문→결과→연락처).
> - 리드 목록·상세·CSV·시트에 계산 결과가 실제로 들어오는지 접수 1건으로 확인.
