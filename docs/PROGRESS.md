...
> ### 🔴 2026-08-08 (gooin PC) — **상태 축 통합 + AS요청 + 선입금 과금 (V29~V31), 브랜치에서 병합 대기**
>
> **브랜치 `feature/unified-status-billing` — main 병합 전 ⚠️ 사용자 확인 필요.**
> 배포되는 순간 **운영 리드 상태가 전부 '신규'로 리셋**된다(사용자 결정 2026-08-08, 옛 값은
> `status_legacy`·`advertiser_status_legacy` 컬럼에 백업만 남는다). 백엔드 테스트 **223개 통과**,
> 프론트 타입체크·빌드 통과. **Flyway V31 까지 → 다음은 V32.**
>
...
>
> #### 병합 전 사용자에게 물어볼 것
> - **배포 = 운영 리드 상태 전부 신규 리셋.** 진행할지 확인. (백업 컬럼은 코드가 안 읽는다 — 되돌리려면 수동 SQL)
> - 배포 후 실물 확인 항목: 광고주 포털 LNB/상태 지정/AS 접수 → 마케터 알림·인정/거부 → 원장 차감/환급,
>   리드폼 편집 정산 카드, 잔액 알림(임계 아래로 내려가는 순간 1회).
>
> #### 다음 사람이 알아야 할 함정
