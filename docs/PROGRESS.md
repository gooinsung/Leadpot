...
>
> **👉 다음 작업 (우선순위 순)**
> 1. **공개 화면 방향 결정** — Cockpit 을 입힐지, 방문자용으로 다른 톤(모바일 퍼스트)으로 갈지 사용자와 상의. UI/UX 단계에서 유일하게 남은 미확정.
> 2. **U6 광고주 포털**(Task-First 홈 + 모바일) — 실제 광고주가 붙을 때 해도 늦지 않다(사용자와 상의).
> 3. (선택) 인박스 rail 에 **기간(직접 지정)·태그 필터** 추가 — 폼별 목록에는 있고 인박스에는 없다(사용자 확인 대기).
> 4. (선택) `RowMenu` 를 다른 목록(리드폼·랜딩·동의문서·요소·IP차단)에도 적용.
>
