...
> - **🔴 SSH 없이 시크릿을 못 고치는 문제** → 웹 UI 로 바뀐다.
>   지금 **Neon 비번 재발급이 이것 때문에 막혀 있다**(아래 "사용자가 처리해야 할 것"). 이전하면 같이 풀린다.
>
> **Vercel 은 검토했으나 탈락** — Spring Boot 가 올라가지 않는다(JVM 런타임 없음). Fly.io 는 차선책. 근거는 계획서 §2.
> ⚠️ **"DB 서울 이전"(아래 사용자 처리 항목)은 이 계획이 대체한다.** 근거는 계획서 §3.
>
> #### 착수하려면 먼저 답해야 할 것 3개 (계획서 §9)
> 1. **R2 판정** — 공개 랜딩 이미지 URL 이 R2 도메인인가 `api.lead-pot.com/uploads/` 인가?
>    (**SSH 불필요** — 브라우저로 확인. `local` 이면 R2 전환이 선행 작업)
> 2. **SSH 키 확보 여부** — `APP_JWT_SECRET` 만은 콘솔에서 못 얻는다. 못 가져오면 **마케터·광고주 전원 재로그인**.
> 3. **Railway 계정·결제** — 사용자가 직접.
>
...
> 4. **SSH 확보** — Neon 비번 교체·운영자 추가·리전 확인이 전부 여기 걸려 있다.
