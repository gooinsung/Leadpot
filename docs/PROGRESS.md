...
> ### 🏢 사업자 정보 표기 + 서비스 소개 페이지 (2026-08-04) — 카카오 알림톡 준비
>
> **왜**: 카카오 **비즈니스 채널 인증이 반려**됨. 사유 = "사업자-채널의 연관성 확인/검증 불가"
...
>
> **다음에 할 일**
> 1. **카카오 비즈니스 채널 인증 재신청** — 위 URL 첨부. 승인까지 며칠 소요.
> 2. 승인되면 **솔라피 채널 연결(발신프로필)** → 관리자 휴대폰 인증 필요(개통 대기 중이었음).
> 3. 그 다음 **알림톡/SMS 발송 모듈**(백로그 H2·H3·F5) — `integration_settings` 에 솔라피 키·발신번호
>    추가(다음 Flyway **V20**), `NotificationService` 의 텔레그램/시트 패턴을 그대로 확장.
>    ⚠️ 광고성 발송은 `(광고)` 표기·080 수신거부·21~08시 금지 규제 대상. 알림톡(정보성)은 해당 없음.
>
> **보류(사용자 결정으로 뒤로 미룸)**
...
>
> **⏸ 세션 마감 (2026-07-31 · gooin PC). 다른 PC/세션에서 이어받는다.**
> - **`main` 최신 = `fb71205`** · 작업트리 클린 · 미커밋/미푸시 없음 · **프론트 자동배포 success**(U2 `60fb348` · U3 `fb71205`) → 지금 https://app.lead-pot.com 에 U3까지 반영됨.
> - **이 세션에서 한 일**: U2 마무리(리드 상세 공용 사이드 패널) → U3 가독성 패스. **백엔드 변경 없음**(프론트 전용, 마이그레이션 없음 — 다음 Flyway는 여전히 **V20**).
> - **이어받으면**: `git pull` → 아래 "이어받는 사람은 여기부터" 1번(실앱 눈으로 확인)부터. 그다음 U4.
>
