...
- **현재 위치**: **핵심 루프(폼 공개→제출→리드 수집→조회) 완성 ✅** — 실제 데이터가 쌓임(방문자정보 포함)
- **완료**: Phase 1 인증 ✅ / Phase 2 폼 빌더 ✅ / **리드 수집(Phase 4 앞당김) ✅** / **Phase 3 랜딩 빌더 & 폼 연결 ✅**
...
> **바로 다음 = 🚨 이미지 업로드**(폼·랜딩 이미지 블록이 지금 URL 입력만 됨 → 파일 업로드 필요). 상세는 아래 "🚨 다음 최우선: 이미지 업로드" 섹션. 파일 저장소(초기 VM 디스크 → 후기 R2/S3) 필요.
> 그 다음: 리드 상태변경·CSV 내보내기(Phase 5 · E1/E2), 구글시트/텔레그램/카톡 알림 연동(리드 저장 훅 자리 있음), 모바일 최적화 확대, 서브도메인(D3), 마케팅 트래킹(Phase 6). 상세는 아래 "플랜에 추가된 항목".
> Phase 0~3 + 리드수집 완료. Flyway 현재 V7까지 사용중(다음 마이그레이션은 V8).
...
## 🚨 다음 최우선: 이미지 업로드 (지금은 URL 입력만 됨 — 실제 파일 업로드 필요)
> 폼·랜딩의 이미지 블록이 현재 **URL 입력만** 가능. 실서비스엔 **파일 업로드**가 필수(FEATURES: 이미지 변경 무제한, MVP). **파일 저장소** 필요.
- **저장 전략(CLAUDE.md)**: 초기 = **VM 디스크(Docker 볼륨 마운트)** → 후기 = Cloudflare R2 / S3. 이식성 위해 저장 추상화 권장.
- 백엔드: `POST /api/uploads`(multipart, 로그인 필요) → 디스크 저장 → 공개 URL 반환. 정적 서빙(`/uploads/**` 또는 공개 엔드포인트). docker-compose에 업로드 볼륨 추가. 용량/확장자/이미지 검증.
- 프론트: 이미지 블록 편집(폼 `FormEditPage`의 IMAGE 블록 + 랜딩 `LandingEditPage`의 IMAGE 블록)에서 **파일 선택 → 업로드 → 반환 URL을 block.url 에 저장**. (URL 직접 입력도 유지 가능)
- 적용 위치 2곳: 폼 빌더 IMAGE 블록, 랜딩 빌더 IMAGE 블록.
