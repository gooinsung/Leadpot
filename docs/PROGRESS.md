...
> **바로 다음 = 리드 관리 고도화**: 리드 **상태 변경**(신규/상담중/완료, E1) + **CSV 내보내기**(E2) → 검색·필터 · 휴지통(E4). (혼자 가능, 계정 불필요)
> 그 다음(혼자 가능): 통계 대시보드(방문자/UTM — 데이터 이미 쌓임) · IP차단(K2) · M8 HTML 요소 라이브러리 · 폼 외부 임베드(M6).
> 사용자 리소스 필요(나중): 구글시트/텔레그램/카톡 연동, SMS 본인인증, 클라우드 배포, 서브도메인/도메인.
> ✅ 이미지 업로드 완료(로컬/VM 디스크). Flyway 현재 V7까지 사용중(다음 마이그레이션 V8).
