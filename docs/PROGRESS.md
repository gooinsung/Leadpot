...
- **✅ 랜딩 HTML 블록용 "선착순 + 실시간 접수" 스니펫 추가 (2026-09-19, 원격 세션, 사용자 요청)**:
  `docs/design/snippets/fcfs-realtime-dark-sky.html` — 플로팅 헤더 + 선착순 카드 + 플로팅 푸터 +
  실시간 접수 토스트를 한 덩어리로 묶은 HTML 블록 스니펫. 사용자가 예전에 티스토리에서 쓰던
  코드를 리드팟 랜딩에 맞게 다시 작성한 것.
  - **디자인**: 검정 배경 + 하늘색(`#38bdf8`) 포인트. 선착순 카드는 `aspect-ratio: 1 / 0.6`
    (원본이 세로로 너무 길었음) — 현재 접수/남은 자리를 가로로 배치해 높이를 줄였다.
    헤더는 "선착순 (현재/전체)명 모집!" 한 줄, 푸터는 헤더와 같은 다크 바 톤(상단 하늘색 라인).
    인라인 CTA 버튼·화살표는 사용자 요청으로 뺐다 — 폼으로의 이동은 푸터 탭이 담당.
    카드 내부는 전부 **가운데 정렬**이고, 아랫부분은 문장형(`현재 선착순 N명 남았습니다` /
    `⚡ 조기 마감 임박!` / `(최종 확인시간: …)`)이다 — 사용자가 준 레퍼런스 이미지를 따랐다.
    가운데 정렬이면 양옆이 비어 보이는 문제는 ① 배지 좌우로 뻗는 가는 선, ② 폭 100% 인
    카운터 패널(큰 숫자 + 진행바), ③ 가로로 긴 문장 줄로 채웠다.
    플로팅 푸터는 사용자 요청으로 **제거**(헤더만 고정) — 본문 하단 여백 보정도 같이 삭제.
  - **동작**: 시작 14명 → 5~15초 랜덤 간격으로 1명씩 → 18명에서 정지(`CONFIG`로 조절).
    접수 1건마다 헤더·카드·푸터·토스트가 **같은 순간에** 갱신된다(`render(true)` + `showToast()`).
  - **HtmlBlock 제약에 맞춤**(`packages/public-ui/src/components/HtmlBlock.tsx`):
    블록 CSS 가 자동 격리되므로 `*`·`body` 전역 리셋을 쓰지 않고, 고정 헤더/푸터 여백은 JS 가
    `document.body` 패딩으로 한 번만 확보한다. `@keyframes` 는 격리 대상이 아니라 이름을 전부
    `lpfc-` 로 프리픽스. 블록이 여러 개여도 각자 1회만 초기화되게 `data-lpfc-ready` 가드.
  - **모바일 최적화**: 375px/320px 실측(Chromium) — 가로 스크롤 없음, 카드 내부 오버플로 없음,
    헤더 1줄 고정(노치 `safe-area-inset-top` 반영), CTA·푸터 탭 타깃 56px 이상,
    `prefers-reduced-motion`·`max-width:360px`·가로모드 대응.
  - **압축본으로 교체(2026-09-19)**: "코드가 너무 길다 / 주석도 싹 빼라"는 요청으로 380줄 →
    **85줄**로 줄였다. 주석 0줄, CSS 1규칙 1줄, 클래스 접두사 `lpfc-` → `lf-`.
    **주석으로 적어뒀던 제약은 아래로 옮긴다**(코드에는 더 이상 안 적혀 있으니 여기를 봐야 한다):
    - `HtmlBlock` 이 블록 CSS 를 자동 격리하므로 `*`·`body` 전역 리셋을 쓰지 않는다. 고정 헤더에
      본문이 가리지 않게 하는 여백은 JS 가 `document.body` 패딩으로 1회만 넣는다(`C.pad`).
    - `@keyframes` 는 격리 대상이 아니라 이름을 전부 `lf-` 로 프리픽스했다(`lf-bl`·`lf-gl`·
      `lf-pop`·`lf-fl`). 다른 블록과 이름이 겹치면 서로 덮어쓴다.
    - 블록이 여러 개여도 1회만 초기화되게 `.lf:not([data-on])` 로 고른다.
    - `position:fixed` 는 조상에 `transform`/`filter` 가 있으면 그 조상 기준이 된다 — 헤더가
      화면에 안 붙으면 감싸는 블록의 transform 을 확인한다.
    - 카드가 `aspect-ratio:1/.6` 라 세로가 빠듯하다. `.lf-pn` 의 `min-height:0` 이 빠지면
      320px 에서 내용이 넘친다(압축 중 실제로 한 번 깨졌다).
    - 🔴 **게이지바(`.lf-tr`)는 반드시 `flex:none`** 이어야 한다. 세로 flex 안의 빈 요소라
      자동 최소 크기가 0이고, 카드 높이가 비율로 고정돼 있어 **공간이 조금만 모자라면 게이지바부터
      0까지 찌그러져 사라진다**. 실제로 사용자 랜딩에서 게이지바만 안 보이는 제보가 있었고,
      `HtmlBlock` 격리를 그대로 재현한 테스트에서 375px 기준 높이가 9px→7.8px 로 눌리는 것을
      확인했다(폰트가 다른 환경이면 0). `flex:none` + `min-height` + 패널 여유 확보로 고쳤다.
    - 🔴 **카드는 `aspect-ratio` 를 직접 쓰지 않는다.** 비율로 높이를 못 박으면 세로가 모자랄 때
      내용이 찌그러진다(게이지바가 0이 되거나, 큰 숫자가 제 박스를 넘쳐 패널 여백이 사라진다 —
      둘 다 실제 제보). `min-height:min-content`(·`fit-content`·`max-content`)로는 **못 막는다**
      — 실측 결과 `aspect-ratio` 가 이긴다. 그래서 **CSS 그리드 1칸에 비율 스페이서(`.lf-ar`,
      `aspect-ratio:1/.6`)와 내용(`.lf-in`)을 겹쳐** 행 높이 = `max(비율높이, 내용높이)` 가
      되게 했다. 즉 **비율은 하한**이고 내용이 많으면 카드가 세로로 늘어난다.
      `padding` 은 `.lf-card` 가 아니라 `.lf-in` 에 둬야 바깥 박스 비율이 0.6 으로 유지된다.
    - 검증: 320·375·414·1400px × 본문 폰트 +0%/+20%/+45% 12개 조합 전부 통과 —
      기본 비율 0.602, 폰트를 키우면 0.63~0.67 로 **늘어나고** 잘리지 않는다.
  - **⚠️ 주의**: 랜딩이 `google_ads_safe` 면 `sanitizeHtml` 이 `<script>` 를 제거해 숫자가 멈춘다
    (마크업·디자인은 그대로). 실제 랜딩에 붙여 육안 확인은 아직 안 함.
