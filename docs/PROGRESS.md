...
>
> **👉 다음 작업**
> 1. **⚠️ 미검증 화면 훑기** — 오늘 회귀가 2건 나온 만큼 남은 마케터 화면(통계·연동·IP차단·폼 편집·동의문서 편집)을 **LNB·밀도 적용 후 상태로** 아직 안 봤다. 데이터가 빈 상태도 함께 볼 것.
> 2. **광고주 포털 실화면 확인**(사용자) — U6 검증 한계 참고. 광고주 계정으로 로그인해 할 일 큐·모바일을 눈으로 볼 것.
> 3. **전역 접속 차단 실동작 확인** — 배포 후 실제 IP 를 넣어 랜딩이 404 가 되는지(본인 IP 로 시험하면 스스로 막히니 주의).
