...
- **✅ 폴더 접기 · 만들 때 폴더 지정 · 랜딩의 리드폼 검색 (2026-09-26, 원격 세션, 사용자 지시)**:
  ...
  - **만들 때 폴더 지정**: 새 랜딩·새 리드폼 화면 상단에 폴더 드롭다운(`FolderSelect`). 목록에서 폴더를 고른 채 '새로 만들기'를 누르면 `?folder=` 로 그 폴더가 기본 선택.
    생성 후 기존 `move*Folder` API 로 옮긴다(백엔드 변경 없음). 옮기기만 실패하면 안내 토스트.
  - **리드폼 검색**: 랜딩 편집기 '연결할 리드폼'을 검색되는 드롭다운(`components/SearchableSelect.tsx`, ↑↓·Enter·Esc)으로 교체.

- **✅ 리드 상세 '방문자' 가독성 개선 (2026-09-26, 원격 세션, 사용자 지시)**:
  - **유입**: Referer 헤더는 브라우저 정책상 도메인까지만 와서(`https://sub.lead-pot.com/`) 쓸모가 없었다 →
    리드의 `landingPageId` 로 실제 랜딩 주소(`publicSiteUrl(sub, id)`)를 만들어 링크로 보여준다. 랜딩 없는 리드(폼 직접 링크)는 Referer 그대로.
    백엔드 `LeadResponse` 에 `landingPageId` 추가(DB 변경 없음).
  - **출처**: 한 줄로 이어 붙이던 유입 파라미터를 `캠페인 이름: A캠페인` 형태의 **태그(칩)** 로, 키 라벨 순서(자체 파라미터 먼저)대로 표시.
  - **코드**: `LeadSidePanel.tsx`, `leads.css`(`.ip-utm-*`), `api/client.ts`, `LeadResponse.java`.

- **✅ 외부 API 전달(아웃바운드): 텐핑 연동 대응 (2026-09-26, 원격 세션, 사용자 지시)**:
  - **시스템 값 추가**: `userAgent`(브라우저 정보) · `referer`(유입 경로) — 이미 리드에 저장되던 값을 전달 파라미터로 고를 수 있게.
  - **답변 항목 '숫자만(하이픈 제거)' 옵션**(연락처 항목을 골랐을 때만 보임, 체크박스는 줄 아래): 파라미터별 `digitsOnly` — 텐핑 `{#ITEM2_NOH#}` 처럼 하이픈 없는 연락처용
    (공개 폼 3칸 연락처는 `010-1234-5678` 로 저장된다).
  - **텐핑 설정법**: URL `http://api.tenping.kr/OutBox/Register` · POST · `cid`/`did` 고정값 · `accessip`=접속 IP ·
    `urlreferrer`=고정값(랜딩 URL) 또는 유입 경로 · `useragent`=User-Agent · 숫자 파라미터(63919 등)=답변 항목
    (ITEM1~4 ↔ 항목 매핑은 사용자가 텐핑 캠페인 화면에서 직접 확인).
  - ⚠️ 텐핑이 실패에도 HTTP 200 을 줄 수 있다 — 첫 몇 건은 리드 상세의 응답 본문을 직접 확인.
  - **코드**: `OutboundWebhookService.buildParams/builtinValue`, `FormEditPage`(선택지·체크박스). DB 변경 없음(settingsConfig JSONB).

- **✅ 리드 인박스: 기간 직접 지정 + 세그먼트 숫자가 필터를 따르게 (2026-09-24, 원격 세션, 사용자 지시)**:
  - **기간**: '전체 기간/최근 7일/최근 30일' 에 **직접 지정**을 더하고, 기간을 고르면 **시작일 ~ 종료일** 날짜 칸이 열린다
    (프리셋은 두 칸을 채워주는 지름길. 날짜를 고치면 '직접 지정'으로 바뀐다. 한쪽만 채우면 그쪽 끝만 막힌다).
  - **전체·오늘·미확인 숫자**: 예전엔 필터와 무관하게 내 모든 리드 기준이었다 → 이제 **걸어둔 필터(폼·분야·상태·기간·유입·검색) 안에서** 센다.
    "A 리드폼 + 9/1~9/15" 면 전체 = 그 조건의 리드 수. **'오늘'은 기간을 덮어쓰지 않고 기간 안에서 좁힌다**(API `today=true`,
    기간에 오늘이 없으면 0). 드롭다운 옵션 숫자(폼별·상태별·분야별)는 옵션 목록이라 전체 기준 그대로 — 걸러서 사라지면 다른 값으로 못 바꾼다.
  - **코드**: `LeadService.inbox`(today 인자 + scoped 카운트, 기존 시그니처는 오버로드로 유지), `LeadController`(`today` 파라미터),
    `LeadInboxPage`·`api/client.ts`(`today`), `leads.css`(`.il-dates`). DB 변경 없음.
  - **테스트**: `InboxTest` 에 폼 필터·기간 카운트 2건 추가 → `com.leadpot.lead.*` 32개 통과. 프론트 build 통과.
    (메모: 이 세션에서 Maven Central 이 429 를 내서 Google 미러 init 스크립트로 돌렸다 — 저장소 설정은 안 건드림.)

- **✅ 입력 항목에 라디오버튼·체크박스 추가 + 기본형·스텝형 입력 유형 단일화 (2026-09-23, 원격 세션, 사용자 지시)**:
  - **새 유형**: 라디오버튼(단일 선택) · 체크박스(중복 선택). 선택지·'기본 선택'은 선택박스와 같은 `options.choices`/`options.defaultIndex`.
  - **단일화(사용자 요구: "기본형에 새 입력 타입이 생기면 스텝형에도 당연히 적용")**: 입력 유형 목록이
    `packages/public-ui/src/lib/fieldTypes.ts` 의 `INPUT_FIELD_TYPES` **한 곳**에 있다.
    기본형 유형 = 이 목록, 스텝형 답변 방식 = 카드형(single/multi, 스텝형 전용) + 이 목록.
    스텝형 질문은 `choiceAsField()` 로 입력 항목 모양으로 바꿔 **기본형과 같은 컴포넌트**로 그린다
    (공개 폼 `LiveField`, 편집기 미리보기 `FieldView`). 검증·접수 값·기본 선택도 같은 함수(`fieldBlockError`·`fieldAnswer`·`fieldDefaultValue`).
    → 새 유형을 넣을 때 손볼 곳: `INPUT_FIELD_TYPES` + `LiveField` + `FieldView` 세 곳뿐, 스텝형은 자동으로 따라온다.
  - **구 스텝형 '단일/다중 선택(목록)'**(`list_single`/`list_multi`)은 라디오·체크박스와 같은 것이라 합쳤다.
    저장된 폼은 `normalizeAnswerType()` 으로 읽을 때 바뀌어 그대로 동작하고, 다시 저장하면 새 이름으로 저장된다.
  - **동작 변화(스텝형)**: 연락처 답변이 기본형처럼 3칸 전화번호 입력(`PhoneInput3`)이 된다. 라디오는 목록형처럼 고르면 바로 다음 단계로 넘어간다.
  - **저장 형식**: 라디오·체크박스 답변 값은 고른 라벨을 선택지 순서대로 `", "` 로 이은 문자열. 공개 폼 내부에서는 인덱스("0,2")로 들고 있다가
    접수할 때 라벨로 바꾼다(라벨에 쉼표가 있어도 안전). **DB·백엔드 변경 없음.**
  - 수기 등록 창도 같은 규칙: 라디오·선택박스·카드 단일 = 드롭다운, 체크박스·카드 다중 = 체크 목록.
  - **테스트**: `fieldTypes.test.ts`(5) 추가. 프론트 build·lint·vitest(21), public-ui vitest(86), renderer `next build` 통과.
  - **⬜ 남은 것**: 실제 화면 육안 확인(이 세션엔 백엔드 DB 없음).
...
- **✅ 공개 랜딩 URL 을 서브도메인으로 되돌림 + '구글 광고용' 체크박스 UI 제거
  ...
  **그 뒤로도 구글 광고 승인은 끝내 나지 않았다.** 즉 "서브도메인 구조가 원인"이라는 가설이
  검증되지 않은 채 URL만 길어진 상태였다. 사용자가 **구글 광고를 포기**하기로 결정하고
  (메타·당근·카카오·틱톡은 영향 없음), 고객 신뢰도 면에서 `the-law.lead-pot.com/37` 이
  `go.lead-pot.com/the-law/37` 보다 낫다고 판단해 **서브도메인으로 복귀**를 지시.
  - **구 `go` URL 은 지우지 않는다** — 폐지 기간에 각 광고 플랫폼 Final URL이 이미 `go` 형식으로
    갱신됐다(사용자 확인). 지우면 실행 중인 광고가 즉시 깨진다. 그래서 **방향만 뒤집었다**:
    예전 `{sub} → go` 301 이 이제 `go → {sub}` 302 다.
  - ⚠️ **301이 아니라 302로 한 이유**: 2주 사이 URL 구조를 두 번 뒤집었다. 301(영구)은 브라우저가
    사실상 무기한 캐시하므로, 지난 전환 때 심어둔 `{sub}→go` 301 캐시를 가진 브라우저가 이번
    `go→{sub}` 와 만나면 **무한 리다이렉트 루프**(ERR_TOO_MANY_REDIRECTS)가 된다. 여기서 또 301을
    심으면 다음에 손댈 수 없다. 공개 랜딩은 재방문이 거의 없어 캐시 이득도 사실상 없다.
    사용자 판단: "애초에 우리 도메인을 쓰는 사람이 거의 없어 상관없다."
    → **테스트는 시크릿 창에서 할 것.** 일반 창에서 루프가 보이면 지난 301 캐시 탓이다.
  - **고친 곳**: `renderer/src/proxy.ts`(라우팅 방향 전환 — `LEGACY_PUBLIC_SITE_HOST="go"` 가
    이제 302 리다이렉트, 서브도메인이 `/site/{sub}/{id}` 재작성으로 콘텐츠 서빙. 재작성 코드는
    eff639c 이전 구현을 그대로 복원해 한글 슬러그 이중 인코딩 재발을 막음) ·
    `packages/public-ui/src/lib/site.ts`(`publicSiteUrl()` — URL 형식 단일 출처) ·
    `renderer/.../site/[subdomain]/[identifier]/page.tsx`(canonical·og:url) ·
    관리 앱 안내 문구 3곳(`DashboardPage`·`LandingEditPage`·`PublicLandingPage`).
  - **백엔드·DB 변경 없음** — `users.subdomain` 컬럼도 `resolveSite(subdomain, identifier)` API
    계약도 09-09 때부터 그대로였다. 그래서 양방향 전환이 전부 순수 코드 변경으로 끝난다.
  - **인프라 변경 없음** — 와일드카드(`*.lead-pot.com` DNS + Workers 라우트)가 양쪽을 다 덮는다.
  - **'구글 광고용' 체크박스는 UI만 제거, 기능은 유지**(사용자 지시 "버튼 빼고 기능만 남겨둬"):
    `LandingEditPage.tsx` 에서 체크박스와 설명 문구만 삭제. `googleAdsSafe` 상태는 서버 값을
    읽어(`setGoogleAdsSafe`) 저장 시 그대로 되돌려보내므로 **이미 켜둔 랜딩이 저장해도 안 꺼진다**.
    `sanitizeHtml` 정화 로직·구글 전용 픽셀 필터(`pixels.ts`)·미리보기 반영도 전부 그대로 — 되살릴
    때 체크박스만 다시 붙이면 된다.
  - **검증**: `tsc --noEmit`(public-ui·frontend·renderer) · `vitest`(public-ui 81개·frontend 21개) ·
    `next build` 통과. **`next start` 로 실제 호스트 라우팅 검증**:
    `go/bali/12`→302 `bali.../12` · 쿼리(`utm_source`·`gclid`) 보존 확인 · 한글 슬러그 보존 확인
    (이중 인코딩 없음) · `go/bali`(식별자 없음)→302 후 404 · `bali/12`→재작성되어 백엔드 호출까지
    도달(로컬 백엔드 없어 ECONNREFUSED, 라우팅은 정상) · `bali/`→404 · `app/login`→관리앱 프록시
    시도(egress 차단으로 실패, 프록시 경로는 정상) · `www/`→200 플레이스홀더.
  - **⬜ 남은 것(사용자/다음 세션)**: ① `main` 병합 후 실제 `{sub}.lead-pot.com/{id}` 브라우저 접속
    확인(**시크릿 창**) ② 구 `go` URL 이 302 로 잘 넘어가는지 확인 ③ 광고 Final URL 을 서브도메인
    형식으로 되돌릴지는 **선택** — 302가 받아주므로 급하지 않지만, 리다이렉트 한 홉을 없애려면
    갱신하는 게 낫다.
...
- **✅ 리드폼/랜딩 목록 — 체크박스 클릭 시 드래그가 가로채 체크가 안 먹던 버그 수정
  ...
  움직여도 브라우저가 "mousedown 지점에서 가장 가까운 draggable 조상"을 찾다가 `tr`을 찾아
  행 드래그로 처리해버리는 문제(HTML5 DnD 스펙 동작) — 체크박스를 눌러도 반응이 없거나
  멈춘 것처럼 보임. `dragOriginRef`로 실제 mousedown 대상(`e.target`)을 캡처 단계에서
  기록해뒀다가, `onDragStart`에서 그 대상이 `input`/`button`/`a` 안이면 `e.preventDefault()`로
  드래그 자체를 취소하도록 수정 — 체크박스·버튼 클릭은 정상 동작, 행 드래그해서 폴더로
  옮기는 기능은 그대로 유지.
  - **검증**: `tsc --noEmit` 통과. 실제 브라우저 상호작용(체크 후 드래그) 재현 테스트는
    이 세션 환경에 브라우저가 없어 못 했음 — 다음에 이어받으면 `npm run dev` 로 띄워
    `/forms`·`/landings`에서 체크박스 클릭이 정상 토글되는지, 행 드래그→폴더 드롭이
    여전히 되는지 육안 확인 필요.
...
- **✅ 공개 랜딩 SEO(I3, 2026-09-07)**: `renderer/`(SSR)의 `generateMetadata` 에 랜딩별
  ...
  — 실측 확인). 예전엔 CSR 라 못 했던 것, SSR 전환(Phase 3) 덕에 이제 가능해짐. Cloudflare 배포를
  기다리는 동안 독립적으로 진행한 작업.
...
>
> **① 구글 광고가 계속 거절되는 원인을 코드에서 특정했다** (더엘 `the-law.lead-pot.com/37`,
> "시스템 우회"·"손상된 사이트" 반복 거절. 로고 이미지를 다 바꿔도 그대로 → 애셋이 아니라 최종 URL 이 원인):
>   - **시스템 우회** = 우리가 **CSR** 이라 서버가 내려주는 HTML 이 빈 껍데기(`<div id="root">`)다.
>     게다가 M8 동적 요소(`LandingView.tsx:47~62`)가 두 번째 API 왕복 후에 숫자를 덮어써서
>     **심사 시점 화면(`0명`·`100개`)과 실사용자 화면(`37명`·`63개`)이 다르다** → 클로킹과 구분이 안 된다.
>   - **손상된 사이트** = `HtmlBlock.tsx:98` 의 `host.innerHTML = html` 이 마케터가 붙여넣은
>     **임의 `<script>` 를 그대로 실행**한다(+ CSP 부재). 스캐너 입장에선 해킹된 사이트와 구분 불가.
>   - → **[docs/SSR-LANDING-PLAN.md](SSR-LANDING-PLAN.md) 신설.** Next.js(Cloudflare Workers)로 공개 랜딩만
>     SSR 전환하는 7단계 계획. Spring 직접렌더(A)·프리렌더캐시(C) 와 비교해 B 를 고른 이유, IP 차단이
>     깨지는 함정(🔴 `CF-Connecting-IP` 전달 필수) 등 전부 문서에 있다.
>   - **✅ 2026-09-07 핵심 방향 확정(사용자)**: 렌더링은 **전면 SSR 고정**(랜딩별 토글 없음 — 경로
>     이원화·토글 깜빡 방지). HTML 블록 **스크립트 정화는 전면 적용 안 함**(메타·당근·카카오용 랜딩은
>     카운트다운·플로팅배너를 계속 써야 함) — 대신 랜딩별 **"구글 광고용" 체크박스**로 정화 여부 선택.
>     전면 SSR 의 유일한 리스크(렌더러 장애=전체 장애)는 **SSR 실패 시 SPA 셸 자동 폴백**으로 덮음.
>   - **✅ 2026-09-07 §11 남은 질문 3개도 전부 확정**: ① `/f/{id}` 단독 공개 폼은 SSR 범위에서 **제외**
>     (구글 광고 최종 URL 로 안 씀) ② 공유 패키지(`packages/public-ui`) 추출은 **지금** 한다(나중일수록
>     드리프트 비용 커짐) ③ 프론트 호스팅 이전(HOSTING-MIGRATION-PLAN Phase B)을 **이 작업이 흡수** —
>     `app.lead-pot.com`(관리 화면)도 같이 Cloudflare Pages 로 옮기고 안정화 후 Oracle VM 종료.
>   - **📋 SSR-LANDING-PLAN.md 는 이제 열린 질문 없이 착수 가능한 상태다.** 다음 세션은
>     **Phase 0**(어댑터·와일드카드 확인)부터 바로 시작 → Phase 1(공유 패키지 추출)로 진행.
>
> **② 배포 완료된 버그 수정 2건**
>   - `d3b8395` **임베드 다크모드 유입 수정**: `embed.tsx` 루트에 `public-form` 클래스가 없어
>     `public.css` 의 라이트 고정(`color-scheme: light`)이 안 걸렸다 → 방문자 폰이 다크모드면
>     관리자앱용 다크 팔레트(짙은 남색 input)가 그대로 새어 들어갔다. 클래스 추가로 해결.
>   - `992877e` **동의문서 '보기' 인라인 펼침**: 새 페이지(`/consent/{id}`)로 이동하던 걸
>     그 자리에서 스크롤 영역으로 펼치도록 변경(작성 중이던 값이 날아가는 이탈 방지).
>     공개 폼·빌더 미리보기가 `ConsentItemRow` 공용 컴포넌트를 쓴다. 헤드리스 크롬(375px)으로 실동작 검증.
>
> **다음 세션이 할 일**: SSR-LANDING-PLAN.md §11 을 사용자와 결정 → Phase 0 부터.
> 급하면 그 전에 임시 처방으로 **랜딩 #37 의 HTML 블록·동적요소를 걷어내고 재심사(이의신청)** 를 넣어볼 것.

> ## ✅ 2026-09-01 (4) — **강조 빨간색 CSS 우선순위 버그 수정 + 단일 선택 자동 진행 — 배포 완료**
>
> 같은 세션 이어서, 사용자 버그 리포트 2건 연속 처리.
>
> **1) "강조에서 빨간색이 안먹는다"**: 원인은 CSS 로드 순서 — `.dash-sub`(질문 설명, layout.css)의
> `color: var(--muted)`가 `.emphasis-red`(base.css)보다 나중에 로드돼 같은 한 클래스짜리 우선순위
> 싸움에서 이겨 항상 회색으로 덮어썼음(굵게는 dash-sub에 font-weight가 없어서 우연히 정상 동작).
> `form-builder.css`(features 레이어, 더 나중 로드)에 `.dash-sub.emphasis-red`/`-redbold` 컴파운드
> 셀렉터를 추가해 다시 못박아 해결. 커밋 `6a86c04`.
>
...
> 세션이라 계속 못 해봤음. Neon 접속 가능한 다음 세션에서 우선 확인 권장.
...
>
> **✅ 병합·배포(2026-09-01)**: 사용자가 "배포해둬, 기존 랜딩에 영향 안 가잖아"로 확정 → 브랜치를
> `main`에 fast-forward 병합(충돌 없음, 이번 커밋은 프론트 파일만 변경) → push →
> GitHub Actions **Deploy Frontend run #112 success** 확인(`백엔드/DB 변경 없어 deploy-backend는
> 아예 트리거 안 됨`). 커밋 `67485c2`.
>
> **⚠️ 아직 안 한 것(다음 세션은 여기부터)**:
> 1. **실제 앱에서 진짜 브라우저로 종단 플로우 확인**(이 세션은 DB 접속정보가 없는 클라우드
>    세션이라 못 해봄): 리드폼(STEP 유형) 만들기 → 랜딩 만들기 → FORM 블록에서 노출 방식을
>    `fullscreen`으로 선택 → 공개 링크(`/p/{slug}`)에서 커버→CTA→스텝 진행→연락처 입력→
>    제출→리드 저장까지. 배포는 됐지만 이 종단 확인은 아직 안 됨.
> 2. (선택) 마지막 질문 단계의 버튼 문구를 레퍼런스처럼 "다음"→"상담 정보 입력"으로 바꾸는 건
>    `StepFlow`가 다른 화면(카드·오버레이)에서도 쓰는 공용 로직이라 **일부러 손대지 않음**(전체
>    스텝폼 문구가 다 같이 바뀌는 걸 원하지 않을 수 있어 사용자 확인 후 진행).
> 3. 브랜치(참고용, 이미 main에 병합됨): `claude/leadpot-fullscreen-landing-qi2x3y`.
...
> - **✅ (추가 검증) 실제 HTTP 종단 테스트 완료**: 로컬 Postgres(16)+백엔드를 직접 띄워(V39까지 마이그레이션
>   적용 확인) 진짜 curl 로 가입→로그인→리드폼 생성→웹훅 켜기→매핑 저장→메타 형태 페이로드 POST까지
>   전부 확인. 저장된 리드 라벨/varKey/동의 정상, 같은 leadgen_id 재전송 시 멱등(`created:false`), 필수
>   동의 누락 시 거부(200 + 최근 오류 기록), 잘못된 토큰 404, WEBHOOK 폼의 공개 URL 404 — 전부 설계대로
>   동작 확인. 위 §1(아래 있던 "실제 HTTP 레이어 테스트 안 함") 항목 해소.
> - **✅ 사용자 방향 전환 — 구글 스프레드시트+Apps Script 안내를 웹훅 화면에 추가**: "메타 말고 당근 등
>   다른 매체도 시트에 쌓이는 경우가 많다"는 이유로, 벤더 종속 웹훅(LeadsBridge 등) 안내보다 **시트+Apps
>   Script 경로를 주 안내로 쓰기로 결정**. 별도 백엔드 없이 **기존 웹훅 URL을 그대로 재사용**(Apps Script가
>   그 URL로 POST) — `WebhookLeadPanel.tsx`에 "📊 구글 스프레드시트로 연결하기" 체크박스 추가, 체크하면
>   발급된 URL이 자동으로 채워진 Apps Script 코드(복사 버튼) + 단계별 설치 안내(확장 프로그램→Apps
>   Script→붙여넣기→`setupTrigger` 실행→권한 승인, 최대 1분 지연 고지)가 펼쳐짐. 스크립트는 행마다
>   `_rowId`(시트ID+행번호)를 자동 부여해 멱등성 ID로 매핑 가능하게 함(재실행돼도 중복 저장 안 됨).
>
> **⚠️ 아직 안 한 것(다음 세션은 여기부터)**:
> 1. **Apps Script 코드를 실제 구글 스프레드시트에 붙여넣어 진짜로 실행해보는 것** — 지금까지는 코드
>    작성만 했고, 실제 Apps Script 편집기에서 돌려서 트리거 등록·권한 승인·전송까지 실제로 확인한 적은
>    없다. 문법 오류 등은 없는지 실제 구글 환경에서 검증 필요.
> 2. LeadsBridge 는 사용자가 트라이얼(14일) 가입해둔 상태 — 이번 방향 전환으로 우선순위는 낮아졌지만
>    필요하면 그쪽 경로도 여전히 열려 있음(같은 웹훅 URL 재사용).
> 3. `FormsListPage` 등에 WEBHOOK 폼용 "공개 링크" 버튼이 남아있는지 확인(현재 안 막았음, 눌러도 404 라
>    기능상 문제는 없지만 사소한 UX 정리 항목).
> 4. **아직 `main` 미병합·미배포** — 실제 운영에서 쓰려면 PR 만들고 병합·Railway 배포까지 필요.
> 5. (별개 이슈) 사용자가 "기존 리드폼→시트 아웃바운드에서 최근 2건이 안 쌓인 것 같다"고 보고 →
>    직접 다시 넣어보니 정상 동작 확인함(재현 안 됨, 일시적 현상으로 보임 — 이번 웹훅 작업과 무관,
>    브랜치가 아직 미배포라 애초에 영향 줄 수 없었음). 재발하면 그때 다시 조사.
...
> `feature/advertiser-unseen-first-leads`)로 작업해 `main`에 병합·배포함(사용자가 "다 배포해"로 확정).
>
> ### 1) 정산·수집 목표 제거
>
> 사용자 요청: "운영 메뉴의 목표, 리드폼의 정산·목표를 빼자 — 정산 관리는 따로 제대로 만들 계획이다."
> **백엔드·DB까지 완전히 삭제하기로 확정**(AskUserQuestion — UI만 숨기는 안전한 선택지 대신 완전
> 삭제를 선택함, 실제 과금·잔액 데이터가 있었다면 되돌리기 어렵다는 점 고지 후 진행).
>
> - **제거한 것**:
...
>
> ### 2) 광고주 리드 목록 기본값 '미확인' — 콜 우선순위
>
> 사용자 요청: "광고주한테 엑셀이랑 광고주페이지를 같이 줬는데 페이지를 잘 안 쓴다. 접속하면 기본이
> '미확인 리드'였으면 좋겠고, 미확인/확인 구분을 뱃지나 색깔 정도로 보여주고 싶다."
>
> - **기존 데이터 재사용**: `advertiser_seen_at`은 이미 DB·API(`AdvertiserLeadResponse.advertiserSeenAt`)에
>   있었다 — V33 때 "광고주에게 열람 여부를 보여주면 마케터가 감시하는 성적표처럼 보인다"는 이유로
>   화면에서만 안 그렸을 뿐. 이번엔 목적이 다르다(광고주 본인이 콜 우선순위를 정하는 용도)고 판단해
>   **이 화면에서만 V33 결정을 뒤집음** — 백엔드 스키마 변경 없이 필터 하나만 추가.
> - **백엔드**: `AdvertiserLeadService.leads(...)`/`filterLeads(...)`에 `unseenOnly` 파라미터 추가
>   (`advertiser_seen_at is null`). `/api/advertiser/leads?unseenOnly=true`. 정렬은 기존 그대로
>   `createdAt DESC`(변경 없음 — 최신순이 이미 기본이라 "빠른 콜이 유리하다"는 추천과 맞았음).
> - **프론트**: `AdvertiserLeadsPage` 기본값 `unseenOnly=true`. 할 일 큐에 '미확인' 카드를 맨 앞에 추가
>   (숫자는 `dash.unseenLeads`, 기존 필드라 이미 있었음) + 필터 바에 '전체 리드 보기' 체크박스.
>   리드 카드에 `adv-unseen` 클래스(앰버 테두리, CSS는 예전에 준비돼 있었는데 안 쓰이고 있었음) +
>   `badge b-wait`로 "미확인" 뱃지.
> - **테스트**: 신규 `unseenOnlyExcludesViewedLeads`(`AdvertiserLeadAccessTest`).
>
> ### 공통 검증 · 배포
>
> 두 기능 각각 백엔드 전체 테스트 통과 · 프론트 `tsc --noEmit`·`vitest`(81개)·`npm run build` 전부
> 통과. **브라우저 실검증은 안 함**(클라우드 세션, Neon 접속정보 없음). 사용자 확인("저거 목표
> 삭제해도돼 괜찮아" · "다 배포해") 후 `main`에 순서대로 병합해 GitHub Actions(프론트·백엔드) 배포
> 트리거함 — Flyway V38이 배포 시 자동 적용되어 `advertiser_ledger` 테이블과 과금 컬럼이 실제로
> 사라짐(계획대로, 되돌릴 수 없음).
>
> **다음 후보**:
> - ⬜ 배포 결과 확인(Actions 성공 여부·`/api/health`).
> - ⬜ 새 정산 관리 기능 설계(사용자가 "따로 만들 계획"이라고 한 것) — 아직 요구사항 미정.
> - ⬜ 브라우저 실검증(다음 세션, 로컬/Neon 접속 가능한 환경): 미확인 카드 숫자·전체 체크박스 토글·
>   리드 상세 열람 후 배지가 사라지는지(같은 페이지 새로고침 전까지는 목록에 남아있는 게 의도된 동작).
>
> ---
>
...
> 사용자의 리드폼·랜딩·리드를 **읽기 전용**으로 열람(기존 "리드 열람 금지" 정책을 사용자 결정으로 변경).
>
> | # | 완료한 것 | 브랜치 |
> |---|---|---|
> | 1 | **마케터 비밀번호 재설정(V36)** — 로그인 화면 '비밀번호를 잊으셨나요?' → 가입 휴대폰으로 6자리 인증번호 문자 → 새 비번+자동 로그인. 해시만 저장·10분 만료·5회 시도 제한·60초 쿨다운·일 5회 상한. 계정 존재 비노출(요청 항상 204). 본인확인 문자는 V25 권한 검사·월 사용량 집계에서 제외(AUTH) | `feature/marketer-password-reset` |
> | 2 | **운영자 읽기 전용 열람** — `/api/admin/users/{id}/forms·landings·leads`(GET 전용) + `/admin/users/:id` 상세 화면(탭). **리드 열람은 호출마다 감사 로그(LEADS_VIEW)** · detail 개인정보 금지 원칙 유지 · 수정 API 없음. 정책 주석 3곳(AdminService·AdminController·AdminUsersPage) 갱신됨 | `feature/admin-readonly-view` |
> | 3 | **버그 수정: AdminBootstrap 승격이 DB에 저장 안 됨** — 이벤트 리스너 자기호출이라 `@Transactional` 미적용 → 승격 로그만 남고 role=USER 유지. `save()` 명시 호출로 수정 + 트랜잭션 밖 재현 회귀 테스트. 프로덕션 운영자 계정 role 이 실제로 ADMIN 인지 **확인 필요할 수 있음** ⚠️ | (2번 브랜치에 포함) |
>
> **검증**: 백엔드 전체 테스트 ✅ / 프론트 vitest 81개 + build ✅ / **브라우저 검증 ✅** (이 PC 는 WSL/Docker 가
> 없어 H2 파일 DB 로 검증 — `application-local.properties` 를 H2 로 바꾸고 build.gradle 에 h2 runtimeOnly 를
> 임시 추가했다가 되돌림. H2 파일 `C:\Temp\leadpot-verify`). 재설정 2단계 전환·쿨다운·오류 메시지,
> 어드민 목록→상세→리드 탭→감사 이력(리드 열람(전체, 2건)) 전부 화면 확인.
>
> **다음 후보**:
> - ⬜ 어드민 확장 나머지(리스트업 문서 참고): 계정 생성/정지/비번재설정, 운영 토글 화면화, SMS 비용 대시보드
> - ⬜ 프로덕션 배포 후 확인: Flyway V36 적용, 재설정 문자 실발송(솔라피), 부트스트랩 재기동 시 운영자 role
>
> ---
>
