...
- **완료**: Phase 1 인증 ✅ / Phase 2 폼 빌더 ✅ / **리드 수집(Phase 4 앞당김) ✅** / **Phase 3 랜딩 빌더 & 폼 연결 ✅** / **리드 상태변경·CSV 내보내기 ✅** / **이미지 업로드 R2 연동(경로 구조화 landing-image/YYYY/MM/DD) ✅** / **통계 고도화(방문 추적+전환율+기간/대상 필터+랜딩/폼별) ✅**
...
> ✅ 이미지 업로드 완료(**R2 연동**, 경로 landing-image/YYYY/MM/DD). ✅ 리드 상태변경·CSV. ✅ 통계 고도화(방문/전환율/필터/랜딩·폼별). Flyway 현재 **V8(visits)** 까지 사용중(다음 V9).
> 🌐 **서브도메인 URL(D3)**: 사용자가 원함 → 앱-사이드는 *.localhost 로 로컬 구현 가능, DNS/SSL은 배포 시점. 방향 질문 중 사용자가 보류(대기). 재개 시 루트 동작/경로 식별자/기존 /p/ 유지 여부 확정 후 착수.
