...
> ### ⏸ 2026-08-02 세션 (gooinsung PC) — 문자 발송 시스템 1차 완료
>
> **Flyway V23 까지 → 다음 V24** · 백엔드 테스트 전체 통과 · 프론트 타입체크·빌드 통과
> 계획 정본은 [MESSAGING-PLAN.md](MESSAGING-PLAN.md) (§9 알림톡 범위 · §10 솔라피 근거 · §11 발송 계정 구조)
>
> #### 이 세션 결과
> 1. **로고 교체** — `LeadPot` 워드마크(`Lead` 그린 + `Pot` currentColor). 팟 아이콘·`.mark` 제거.
>    파비콘은 네이비 반전, 카카오 채널용 640x640 은 2줄 스택([docs/design/](design/)).
> 2. **M1 변수키(V22)** — `form_blocks.var_key` + `forms.var_key_seq`. 지운 키는 재사용하지 않는다.
>    리드 접수·CSV 임포트 시 `answers` 에 `varKey` 기록. 스텝형은 저장 시 CHOICE 를 재조립하므로
>    `StepData` 에도 varKey 를 들고 있게 했다(안 하면 저장마다 키가 새로 발급돼 템플릿이 깨진다).
> 3. **문자 발송 시스템(V23)** — 솔라피 어댑터 + 트리거 3종 + 현황·이력 화면.
>
> #### ⚠️ 배포 전에 반드시 할 일 (안 하면 문자가 안 나간다)
> **서버 `.env` 에 리드팟 솔라피 자격증명을 넣어야 한다.** 값이 없으면 발송이 전부 `SKIPPED` 로 기록된다.
> ```
> APP_SMS_SOLAPI_API_KEY=...
> APP_SMS_SOLAPI_API_SECRET=...
> APP_SMS_SOLAPI_SENDER_PHONE=01012345678   # 솔라피에 사전등록된 번호
> ```
> 선택: `APP_SMS_MONTHLY_LIMIT_FREE`(기본 100) · `APP_SMS_MONTHLY_LIMIT_PRO`(기본 5000)
>
> #### 아직 검증 못 한 것 (자격증명이 없어서)
> - **실제 발송을 한 번도 못 해봤다.** 솔라피 요청 본문(`{"message":{...}}`)과 응답 파싱은 문서 기준으로 맞췄지만
>   실물 확인 전이다. **`/sms` 화면의 "테스트 발송" 버튼으로 가장 먼저 확인할 것.** 실패 사유가 화면에 그대로 나온다.
> - 발신번호 회선 미개통 → [MESSAGING-PLAN.md](MESSAGING-PLAN.md) §11 "알뜰폰 회선 개통 절차" 참고(eSIM 권장).
>
> #### 다음에 할 일 (우선순위 순)
> 1. **실발송 검증** — 위 자격증명 넣고 `/sms` 테스트 발송. 실패하면 요청 본문·엔드포인트부터 확인.
> 2. **리드 상태 변경 트리거** — 지금은 접수 직후만 동작한다(`LeadSmsPlanner.plan`).
> 3. **템플릿 CRUD(M2)** — 지금은 리드폼별 본문 1개(`settingsConfig.smsLeadBody`)다. 여러 리드폼에서
>    재사용하려면 `message_templates` 가 필요하다.
> 4. **변수 사라짐 안전장치** — 리드폼 저장 시 템플릿이 쓰는 변수키가 없어지면 발송을 끄고 이유를 보여준다
>    (MESSAGING-PLAN §2). **아직 미구현이라 지금은 변수가 조용히 빈칸으로 나간다.**
> 5. **메일 발송(Resend)** — 문자 뒤로 밀어둔 단계.
> 6. **알림톡(M7)** — 심사 신청 완료. 통과되면 붙인다(시스템 변수만 쓰므로 M1 과 무관).
>
> #### 확인 필요 (사용자 판단)
> - **광고주 포털 로고** — 원래 `[팟 아이콘] 마케터명` 이던 자리가 `LeadPot 마케터명` 으로 나란히 놓인다.
>   화이트라벨 화면이라 리드팟 로고를 빼는 게 맞을 수 있다(임의로 바꾸지 않았다).
> - **고객향 문자의 발신번호** — 우리 키로 보내면 발신번호가 리드팟 번호가 되어 고객 회신이 우리에게 온다
>   (MESSAGING-PLAN §11). 마케터 번호로 보내려면 위임 서류가 필요하다.
