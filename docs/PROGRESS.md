...
> **기능 자체를 막는 장치가 없다.**
>
> **설계 선택지 (내일 착수 전 사용자와 결정)**
> - **A. `users.sms_enabled` 컬럼 추가(Flyway V25)** — 계정별 on/off. 기본 `false`, 내 계정만 `true`.
>   나중에 관리자 화면에서 토글로 열어주기 좋다. **권장.**
> - **B. 요금제(Plan)로 제한** — PRO 만 허용. 이미 있는 구조라 마이그레이션이 없지만
>   "나만 쓰기"에는 과하고, 유료 결제를 붙이기 전엔 의미가 약하다.
> - **C. 환경변수 화이트리스트**(`APP_SMS_ALLOWED_OWNER_IDS=1`) — 마이그레이션 없이 즉시.
>   대신 대상이 바뀔 때마다 재배포해야 한다.
>
> **⚠️ 막아야 하는 지점 (UI만 숨기면 API 로 뚫린다 — 전수로 처리할 것)**
> 1. **`SmsService.send()`** — 최종 관문. 권한 없으면 **발송하지 말고 `SKIPPED` 로 사유를 남긴다**
>    (조용히 사라지면 안 된다는 기존 원칙 유지).
> 2. **`SmsController`** — `POST /api/sms/test`(테스트 발송), **첨부 업로드**(`uploadAttachment` 는
>    솔라피 저장소에 실제로 올라가므로 반드시 같이 막는다). 현황·이력 **조회는 열어둬도 된다.**
> 3. **리드폼 저장(`FormService.update`)** — `settingsConfig.smsMarketerEnabled` 등이 `true` 로 들어오면
>    **서버에서 거부하거나 강제로 false 처리.** 화면만 숨기면 API 직접 호출로 켤 수 있다.
> 4. **화면** — 리드폼 편집의 '문자 발송' 카드와 `/sms` 진입을 권한 없는 계정에는 숨기고 안내 문구를 띄운다.
>
...
>
> **다음 단계 (솔라피 콘솔 → 코드 순서. 콘솔 작업이 끝나야 코드를 붙일 수 있다)**
> 1. **채널 연동** — 솔라피 콘솔 > 채널/그룹 > 채널 개설·연동. 검색용 아이디(@채널)와
>    **채널 관리자 휴대폰번호가 카카오 쪽과 일치**해야 연동된다. 성공하면 **`pfId`(채널 ID)** 가 발급된다.
> 2. **템플릿 등록 + 검수** — 콘솔 > 발송준비 카카오/네이버/RCS > 알림톡 템플릿 > 등록.
>    **채널 연동 전에는 템플릿 등록 자체가 불가**하다. 본문 초안·버튼은 §9 에 이미 있다.
>    ⚠️ **등록 완료된 템플릿은 수정 불가** — 검수 요청 전에 확정할 것. 검수는 카카오가 하며 영업일 1~3일.
>    ⚠️ **정보성만 허용**(광고성 문구는 반려).
> 3. **대체발송(SMS/LMS) 설정** — 템플릿 등록 화면에서 지정한다. 실패 시에만 문자로 나가므로
>    비용 관점에서 켜 두는 게 유리하다.
> 4. **코드** — 발송 API 는 문자와 같은 `POST /messages/v4/send` 에 `kakaoOptions` 를 얹는다:
>    `{pfId, templateId, variables:{"#{변수}":"값"}, disableSms:false}`.
>    → `SolapiSmsSender` 에 알림톡 분기를 넣거나 `AlimtalkSender` 를 따로 두면 된다(인터페이스는 이미 분리돼 있다).
>
...
>
> **원인은 DB 위치다.** Neon 이 **`ap-southeast-1`(싱가포르)**, 서버는 **서울** → 쿼리 1회마다 170~280ms.
