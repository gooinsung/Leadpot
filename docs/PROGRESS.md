...
>
> ### ⏳ 착수 전 사용자 결정 대기 (CLAUDE.md §0)
> 1. **입력 매핑 방식** — 질문↔계산기입력을 마케터가 수동 매핑? vs 계산기 넣으면 질문 스텝 자동 생성?
> 2. **결과 표시 수준** — 탕감액만 크게? vs 산출 근거(생계비·가용소득·어느 기준이 이겼는지) 함께?
> 3. **면책 문구** — 우리가 강제? vs 마케터가 편집 가능? (변호사·법무사 광고 규제 리스크)
