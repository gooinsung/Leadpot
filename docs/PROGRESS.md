...
>
> **내일 1순위 = 바로 아래 🔒 블록(문자 발송 권한·채널·횟수 제한).** 설계는 이미 확정돼 있고,
> 착수 시 정할 건 **허용 채널을 CSV 한 칸으로 둘지 boolean 3개로 둘지** 하나뿐이다.
> 마이그레이션 V25 배포 후 **운영자 계정만 수동 UPDATE** 하는 것까지가 한 묶음이다.
>
> #### 오늘 배포된 것 (전부 라이브)
> | 커밋 | 내용 |
> |---|---|
> | `e1d2fcf` | 🔴 **리드폼 저장 500 수정** — 변수키 유니크 위반(INSERT 가 DELETE 보다 먼저 실행) |
> | `e5d5e0a` | 사업자 정보 푸터 전역화(ServiceLayout) + 정렬 정리, '꾸스가 운영하는' 문장 제거 |
> | `b451a7a` | 동의문서 '보기' 링크를 앱 도메인 절대 URL로 고정(서브도메인·임베드 404 해결) |
> | `37ea4e7` | 알림 링크 `/leads` → `/inbox` 라우트 연결(문자·알림톡 버튼이 대시보드로 튕기던 문제) |
> | 이 커밋 | 커넥션 풀(HikariCP) 유휴 대응 + 이 문서 |
>
