...
>
> **⚠️ 남은 문제 — 블록 `<style>` 은 전역이다**: 같은 문서 주입의 대가. 사용자가 워드프레스·티스토리에서 쓰던 코드에는 **타 플랫폼용 전역 리셋**이 들어 있다(`* { margin:0;padding:0 }` · `html, body {...!important}` · `body { padding-top:70px !important }` · `#tt-body-page` · `.tt_article_useless_p_margin`). 이게 **랜딩 전체와 관리자 편집 화면까지** 깨뜨린다. 우리 플랫폼에선 그 5줄이 전부 불필요(티스토리 강제 여백을 지우려던 코드) → **지우고 쓰면 된다**. `body{padding-top}` 의 의도(고정 헤더 자리 확보)는 **빌더의 블록 위 여백**으로 대체.
> - **사용자 확인 대기**: 편집 화면에서 전역 선택자(`*`·`html`·`body`)·타 플랫폼 선택자를 **감지해 경고 + "자동 정리" 버튼**을 붙일지. 사용자 CSS 를 말없이 지우는 것은 원인 추적이 어려워 하지 않기로 했다.
>
